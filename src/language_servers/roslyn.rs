...
const TOOL_BINARY: &str = "roslyn-language-server";
...
impl Roslyn {
    ...
    pub fn language_server_cmd(
        ...
        if let Some(path) = worktree.which(TOOL_BINARY) {
            return Ok(zed::Command {
                command: path,
                args: Self::default_args(binary_args),
                env: Default::default(),
            });
        }

        if let Some(server_path) = &self.cached_server_path {
        ...
        let already_installed =
            Self::find_server_path(rid, &version_dir).is_ok_and(|server_path| {
                fs::metadata(server_path.as_str()).is_ok_and(|stat| stat.is_file())
            });
        ...
        if let ServerPath::Exe(path) = &server_path {
    ...
    fn default_args(user_args: Option<Vec<String>>) -> Vec<String> {
    ...
    fn build_command(server_path: &ServerPath, user_args: Option<Vec<String>>) -> zed::Command {
        let extra_args = Self::default_args(user_args);
    ...
    fn transform_settings_for_roslyn(settings: zed::serde_json::Value) -> zed::serde_json::Value {
        ...
        let config_map = roslyn_config
            .as_object_mut()
            .expect("roslyn config should be an object");
...
impl ServerPath {
    fn as_str(&self) -> &str {
        match self {
            Self::Exe(path) | Self::Dll(path) => path,
        }
    }
}
